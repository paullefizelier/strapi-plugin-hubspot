...
const HubspotPropertyInput = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    // Switching the object orphans a property that lives on the previous one
    // (Contact · firstname under Société): validation would refuse the save
    // anyway, so clear it as soon as the *editor* changes the object — never on
    // mount, and never when the property also exists on the new object.
    ...
    const options = React.useMemo(() => {
      ...
      const scoped = selectedObject
        ? all.filter((p) => p.object === selectedObject)
        : all;
    ...
    const describe = (): React.ReactNode => {
      ...
      if (schema && !schema.configured) {
        return formatMessage({
          ...
          defaultMessage:
            "No HubSpot API key — free text. Set it in Settings → HubSpot.",
        });
      }
      if (
        selectedObject &&
        schema?.unavailable.some((u) => u.object === selectedObject)
      ) {
        return formatMessage(
          {
            id: getTranslation("picker.object-unavailable"),
            defaultMessage:
              "Properties of “{object}” unreadable — missing scope on the token.",
          },
    ...
    const crmLink =
      schema?.portalId &&
      value &&
      schema.properties.some((p) => p.name === value)
        ? `https://${schema.uiDomain || "app.hubspot.com"}/property-settings/${
            schema.portalId
          }/properties?search=${encodeURIComponent(value)}`
        : null;
    ...
    const selectedProperty =
      value && schema
        ? schema.properties.find(
            (p) =>
              p.name === value &&
              (!selectedObject || p.object === selectedObject),
          )
        : undefined;
    ...
    const importable =
      selectedProperty?.type === "enumeration" &&
      (selectedProperty.options?.length ?? 0) > 0;
          ...
          <Combobox
            ...
            noOptionsMessage={() =>
              formatMessage({
                ...
                defaultMessage:
                  "No matching property — properties are managed in HubSpot.",
              })
            }
          >
            {options.map((o) => (
